...
public class B2B {
	...
	public static void main(String[] args) throws Exception {
		...
		for (String s : set) {
			...
			// System.out.println(" the minQuote is " + finalItem.getQuote() +
			// " order id " + finalItem.getItemNumber() + " tns " +
			// finalItem.getSupplier() + " quantity " + finalItem.getQuantity());
		}
		...
		for (int i = 0; i < finalList.size(); i++) {
			...
			// System.out.println("\nThe answer is\n" + node.getTextContent());
			finalList.get(i).setConfirmationNumber(node1.getTextContent().substring(node1.getTextContent().indexOf('#')+2));
		
//			System.out.println(node1.getTextContent().substring(node1.getTextContent().indexOf('#')+2));

		}
		in.close();

		finalOrderBean finalOrder = new finalOrderBean(finalList, new Date());
		...
		sw.write("<?xml-stylesheet type=\"text/xsl\" href=\"finalorder.xsl\"?>");
//		sw.write("<?xml-stylesheet type=\"text/xsl\" href=\"../po.xsl\"?>");
		...
		File directory = new File("finalOrder_"+(new Date()).getTime()+".xml");
		...
		if(!directory.createNewFile()){
			...
			//Dumping out contents to error_log
		...
		/*
		 * FileWriter fw = new FileWriter(finalFile); fw.write(sw.toString());
		 * fw.close();
		 */
